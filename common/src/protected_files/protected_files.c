...
#define NUM_FILE_NODE_FREE_LIST_ITEMS 10000

struct file_node_free_list_item {
    struct file_node_free_list_item* next;
    file_node_t file_node;
    bool allocated;
};

struct file_node_free_list_item* file_node_free_list = NULL;
static spinlock_t g_file_node_free_list_lock = INIT_SPINLOCK_UNLOCKED;

pf_status_t init_file_node_free_list(void) {
    assert(!g_initialized);
    struct file_node_free_list_item* item;

    for (size_t i = 0; i < NUM_FILE_NODE_FREE_LIST_ITEMS; i++) {
        item = calloc(1, sizeof(struct file_node_free_list_item));
        if (item == NULL)
...
static struct file_node_free_list_item* allocate_file_node(void) {
    spinlock_lock(&g_file_node_free_list_lock);

    if (file_node_free_list == NULL) {
        spinlock_unlock(&g_file_node_free_list_lock);
        log_error("no free file nodes available; pls adjust the size of the file node free list");
        BUG();
    }
    struct file_node_free_list_item* item = file_node_free_list;
    assert(!item->allocated);
    file_node_free_list = file_node_free_list->next;
    item->allocated = true;
...
static void free_file_node(struct file_node_free_list_item* item) {
    spinlock_lock(&g_file_node_free_list_lock);
    _free_file_node(item);
    spinlock_unlock(&g_file_node_free_list_lock);
}

static void free_file_node_from_data(file_node_t* data_ptr) {
    struct file_node_free_list_item* item = (struct file_node_free_list_item*)((char*)data_ptr -
        offsetof(struct file_node_free_list_item, file_node));
...
static file_node_t* ipf_append_mht_node(pf_context_t* pf, uint64_t mht_node_number) {
    ...
    struct file_node_free_list_item* new_file_mht_node_item = allocate_file_node();
    ...
    if (!lruc_add(pf->cache, new_file_mht_node->physical_node_number, new_file_mht_node)) {
        free_file_node(new_file_mht_node_item);
        pf->last_error = PF_STATUS_NO_MEMORY;
        return NULL;
    }
...
static file_node_t* ipf_get_data_node(pf_context_t* pf, uint64_t offset) {
    ...
    while (lruc_size(pf->cache) > MAX_PAGES_IN_CACHE) {
        ...
        if (!((file_node_t*)data)->need_writing) {
            ...
            free_file_node_from_data(file_node);
        } else {
...
static file_node_t* ipf_append_data_node(pf_context_t* pf, uint64_t offset) {
    ...
    struct file_node_free_list_item* new_file_data_node_item = allocate_file_node();
    ...
    if (!lruc_add(pf->cache, new_file_data_node->physical_node_number, new_file_data_node)) {
        free_file_node(new_file_data_node_item);
        pf->last_error = PF_STATUS_NO_MEMORY;
        return NULL;
    }
...
static file_node_t* ipf_read_data_node(pf_context_t* pf, uint64_t offset) {
    ...
    struct file_node_free_list_item* file_data_node_item = allocate_file_node();
    ...
    if (!ipf_read_node(pf, pf->file, file_data_node->physical_node_number,
        ...
        free_file_node(file_data_node_item);
        return NULL;
    }
...
static file_node_t* ipf_read_mht_node(pf_context_t* pf, uint64_t mht_node_number) {
    ...
    struct file_node_free_list_item* file_mht_node_item = allocate_file_node();
    ...
    if (!ipf_read_node(pf, pf->file, file_mht_node->physical_node_number,
        ...
        free_file_node(file_mht_node_item);
        return NULL;
    }
