...
int _PalVirtualMemoryAlloc(void* addr, uint64_t size, pal_prot_flags_t prot) {
    ...
    if (g_pal_linuxsgx_state.edmm_enabled) {
        ...
        if (g_enclave_page_tracker) {
            ...
            if (g_initial_page_allocs_registered) {
                log_error("unexpected allocation: initial page allocs already registered");
                _PalProcessExit(1);
            }
...
static void register_initial_enclave_pages(void) {
    assert(!g_initial_page_allocs_registered);
    ...
    g_initial_page_allocs_registered = true;
}
...
void initialize_enclave_page_tracker(uintptr_t base_address, size_t memory_size, size_t page_size) {
    ...
    register_initial_enclave_pages();
}
